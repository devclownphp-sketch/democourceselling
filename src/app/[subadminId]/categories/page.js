...
export default async function SubAdminCategoriesPage({ params }) {
    ...
    const canView = canViewSection(rolePermissions, "categories");

    if (!canView) {
        return <UnauthorizedPage sectionName="Categories" username={subadmin.username} />;
    }
    ...
    const canCreate = checkPermission(subadmin, "types.create");
    const canEdit = checkPermission(subadmin, "types.edit");
    const canDelete = checkPermission(subadmin, "types.delete");
    ...
    return (
        ...
        >
            <div style={{ padding: "1rem" }}>
                <h1 style={{ fontSize: "1.5rem", fontWeight: 800, marginBottom: "1rem" }}>📂 Categories</h1>
                <CourseTypeManager
