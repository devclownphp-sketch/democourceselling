...
export default async function SubAdminColorsPage({ params }) {
    ...
    const canView = canViewSection(rolePermissions, "colors");
    const canEdit = checkPermission(subadmin, "colors.edit");

    if (!canView) {
        return <UnauthorizedPage sectionName="Colors" username={subadmin.username} />;
    }
    ...
    return (
        ...
        >
            <div style={{ padding: "1rem" }}>
                <h1 style={{ fontSize: "1.5rem", fontWeight: 800, marginBottom: "1rem" }}>🎨 Colors</h1>
                <div style={{ background: "#fff", border: "4px solid #000", borderRadius: "16px", padding: "2rem", boxShadow: "4px 4px 0 #000" }}>
                    <p style={{ color: "#666", marginBottom: "1rem" }}>Color settings are managed from the main admin panel.</p>
