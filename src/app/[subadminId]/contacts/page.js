...
export default async function SubAdminContactsPage({ params }) {
    ...
    const canView = canViewSection(rolePermissions, "contacts");

    if (!canView) {
        return <UnauthorizedPage sectionName="Contacts" username={subadmin.username} />;
    }
    ...
    const canDelete = checkPermission(subadmin, "contacts.delete");
    ...
    return (
        ...
        >
            <div style={{ padding: "1rem" }}>
                <h1 style={{ fontSize: "1.5rem", fontWeight: 800, marginBottom: "1rem" }}>📧 Contacts</h1>
                <AdminContactsClient
