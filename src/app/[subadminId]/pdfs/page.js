...
export default async function SubAdminPdfsPage({ params }) {
    ...
    const canView = canViewSection(rolePermissions, "pdfs");

    if (!canView) {
        return <UnauthorizedPage sectionName="PDF Notes" username={subadmin.username} />;
    }

    return (
        <SubAdminPageWrapper
            hasAccess={canView}
            sectionName="PDF Notes"
            username={subadmin.username}
        >
            <div style={{ padding: "1rem" }}>
                <h1 style={{ fontSize: "1.5rem", fontWeight: 800, marginBottom: "1rem" }}>📝 PDF Notes</h1>
                <div style={{ background: "#fff", border: "4px solid #000", borderRadius: "16px", padding: "2rem", boxShadow: "4px 4px 0 #000" }}>
                    <p style={{ color: "#666", marginBottom: "1rem" }}>PDF notes are managed through Study Materials.</p>
                    <a href={`/${subadmin.urlId}/study-materials`} style={{ display: "inline-block", padding: "0.75rem 1.5rem", background: "#0084D1", color: "#fff", borderRadius: "12px", textDecoration: "none", fontWeight: 700 }}>
